...
app.post("/api/book-copies", async (req, res) => {
  ...
  const tinhTrangMuon = normalizeTextValue(req.body.tinh_trang_muon) || "Sẵn sàng";
  ...
  try {
    const rows = await runQuery(
      ...
      SELECT
          ...
          bs.ma_chi_nhanh,
          CASE
            WHEN active_loan.ma_sach IS NULL THEN 1
            ELSE 0
          END AS is_available
      FROM BanSao bs
      ...
      WHERE (@available_only = 0 OR active_loan.ma_sach IS NULL)
...
app.post("/api/borrows", async (req, res) => {
  ...
  if (!maThanhVien || !maThuThu || !maSach || !ngayMuon || !ngayHenTra || Number.isNaN(copyNo) || copyNo === null) {
    return res.status(400).json({
  ...
  try {
    ...
    if (activeBorrowRows.recordset[0]) {
